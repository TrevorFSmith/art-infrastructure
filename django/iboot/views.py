...
def index(request):
	return render_to_response('iboot/index.html', {
        'iboots':IBootDevice.objects.all(),
...
def iboot(request, id):
	...
	return render_to_response('iboot/iboot.html', {
        'iboot':iboot,
        'status':control.query_iboot_state(),
        'events':IBootEvent.objects.filter(device=iboot),
